...
def getROIs(items, *args):
    ...
    args: results, align True or False
    saves list with trace data, stim type, behaviour result, roi id,
    trial id, mouse day id, contrast, max speed, 'B' tag
    """
    protocolItem = items[0]
    triggerItem = items[1]
    frameItem = items[2]
    ...
    for c in range(frameItem.childCount()):
        if 'ROI' in frameItem.child(c).text(0):
            # dF/F and Z-score
            data = dfF(frameItem.child(c), zscore=True)
            # Align
            if align:
              if '_Escape' in protocolItem.text(0):
                  data = alignROIs(frameItem, frameItem.child(c), data)
            # Add metadata
            ...
            if tagTimesItem:
                ...
                for c in range(tagTimesItem.childCount()):
                    videoTags.append(str(tagTimesItem.child(c).text(0).split('_')[1]))
                if ("B" in videoTags):
                    tagTime = getTagData(tagTimesItem, 'B')[0]
                else:
                    tagTime = 10000
                speedItem = aux.getItemFromPath(['speed'],frameItem)
                if speedItem:
                    ...
                    maxSpeed = np.max(speedData[int(tagTime/dt):int(tagTime/dt+2000/dt)])
                if tagTime==10000:tagTime = np.NaN
            results.append([data,metadata[0],metadata[1],idROI,
                           idTrial,idMouseDay,contrast, maxSpeed, tagTime])

def alignROIs(frameItem, item, data):
